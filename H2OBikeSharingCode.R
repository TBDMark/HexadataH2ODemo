...
dat <- BikeTrain
dat_h2o <- h2o.importFile(localH2O, "C:/Users/Mark Stephenson/Documents/Two Bytes Data/Kaggle/Kaggle_BikeSharing/train.csv")

## Split 60/40 for Training/Test Dataset
set.seed(1234)
y_all <- as.matrix(dat_h2o$count)
...
for (n in 1:n_run) {
  ...
  ## Train the model
  model <- h2o.deeplearning(x = 2:9,  # column numbers for predictors
                            y = 12,   # column number for label
